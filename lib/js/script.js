...
if(typeof $.cookie('clientSeed') === "undefined"){
    var clientSeed = getRandCseed(); //crypto random
...
$(function(){
    ...
    var config = {
        app_id: 584,
        access_token: ((access_token!="" && access_token!=null)?access_token:(typeof $.session.get('session_access_token') != "undefined"?$.session.get('session_access_token'):undefined)),
    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            if(access_token!="" && access_token!=null){
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+access_token+"&app_id=584&wager=4500000&limit=11&order_by=desc", function(json){
    ...
    socket.on('new_bet', function(payload) {
        if($('#username').text() == payload.uname){
            var table = document.getElementById("game_mybets_logs");
            ...
            cell2.innerHTML = parseFloat(parseFloat(payload.wager/100).toFixed(2))+" Bits";
...
$('#viewMyBetsButton').click(function(){
    $('#game_mybets_log').css("display", "block");
    $('#game_allbets_log').css("display", "none");
});

$('#viewAllBetsButton').click(function(){
    $('#game_mybets_log').css("display", "none");
    $('#game_allbets_log').css("display", "block");
});
